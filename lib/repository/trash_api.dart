...
class TrashApi implements TrashApiInterface {
  ...
  static initialize(
      AppConfigProviderInterface configProvider, http.Client httpClient) {
    if (_instance != null) {
  ...
  Future<RegisterResponse?> registerUserAndTrashData(
      List<TrashData> allTrashData) async {
    _logger
        .d("Register user and trash data@${this._mobileApiEndpoint}/register");
    _logger.d(jsonEncode(allTrashData));
    Uri endpointUri = Uri.parse("${this._mobileApiEndpoint}/register");
    http.Response response = await this._httpClient.post(endpointUri,
  ...
  Future<TrashSyncResult> syncTrashData(String userId) async {
    Uri endpointUri =
        Uri.parse("${this._mobileApiEndpoint}/sync?user_id=$userId");
    http.Response response = await this._httpClient.get(endpointUri, headers: {
    ...
    if (response.statusCode == 200) {
      try {
        ...
        return TrashSyncResult(trashDataList, trashResponse.globalExcludes,
            trashResponse.timestamp, TrashApiSyncStatus.SUCCESS);
      } catch (e) {
