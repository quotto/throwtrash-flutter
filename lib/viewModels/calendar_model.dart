...
class CalendarModel extends ChangeNotifier {
  ...
  final List<List<List<DisplayTrashData>>> _calendarsTrashList = [];
  final List<List<int>> _calendarsDateList = [];
  ...
  List<List<List<DisplayTrashData>>> get calendarsTrashList =>
      _calendarsTrashList;
  ...
  final CalendarService _calendarUseCase;
    ...
    _trashDataService.refreshTrashData().then((_) async {
      for (int i = 0; i < 5; i++) {
        ...
        _calendarsTrashList.add(
          _getDisplayTrashList(calcYear, calcMonth, tmpCalendarDate),
        );
        calcMonth++;
      }
      notifyListeners();
  ...
  void forward() {
    ...
    _currentPage++;
    _logger.d(
      'currentPage: $_currentPage,month:$_month, length: ${_calendarsDateList.length}',
    );
    ...
    if (_currentPage == _calendarsDateList.length - 2) {
      ...
      _logger.d('append calendar: $nextMonth');
      List<int> tmpCalendarDate = _calendarUseCase.generateMonthCalendar(
        nextYear,
        nextMonth,
      );
      _calendarsDateList.add(tmpCalendarDate);
      _logger.d(tmpCalendarDate);

      _calendarsTrashList.add(
        _getDisplayTrashList(nextYear, nextMonth, tmpCalendarDate),
      );
      ...
      _currentPage--;
      _logger.d(
        'removed first, currentPage: $_currentPage,length: ${_calendarsDateList.length}',
      );
    }
  ...
  void backward() {
    ...
    if (_currentPage == 1 && _calendarIndex > 1) {
      ...
      _logger.d('insert calendar: $beforeMonth');
      List<int> tmpCalendarDate = _calendarUseCase.generateMonthCalendar(
        beforeYear,
        beforeMonth,
      );
      ...
      _calendarsTrashList.insert(
        0,
        _getDisplayTrashList(beforeYear, beforeMonth, tmpCalendarDate),
      );
  ...
  List<List<DisplayTrashData>> _getDisplayTrashList(
    int year,
    int month,
    List<int> dateList,
  ) {
    return _trashDataService
        .getEnableTrashList(year: year, month: month, targetDateList: dateList)
        .map(
          (List<TrashData> weekList) => weekList
              .map(
                (TrashData trashData) => DisplayTrashData(
                  trashData.type,
                  _trashDataService.getTrashName(
                    type: trashData.type,
