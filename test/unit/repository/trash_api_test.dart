...
@GenerateNiceMocks(
    [MockSpec<http.Client>(), MockSpec<AppConfigProviderInterface>()])
void main() {
  MockAppConfigProviderInterface appConfigProvider =
  ...
  group("registerUserAndTrashData", () {
    test("registerUserAndTrashDataで正常に登録できること", () async {
      ...
      trashData.add(TrashData(
          ...
          type: "bottole",
          trashVal: "test_trash_id",
          schedules: [TrashSchedule("biweek", "3-2")],
          excludes: []));
      trashData.add(TrashData(
          id: "4",
          type: "other",
      ...
      RegisterResponse? result =
          await trashApi.registerUserAndTrashData(trashData);
      expect(result, isNotNull);
      expect(result!.id, "test_user_id");
      expect(result.timestamp, 12345678);

      final captured = verify(httpClient.post(captureAny,
    ...
    test("registerUserAndTrashDataでレスポンスボディにtimestampが存在しない場合はnullが返ること",
        () async {
      when(httpClient.post(any,
              body: anyNamed("body"), headers: anyNamed("headers")))
          .thenAnswer((request) async {
  ...
  group("updateTrashData", () {
    test("updateTrashDataで正常に更新できること", () async {
      ...
      TrashUpdateResult result = await trashApi.updateTrashData(
          "test_user_id", trashData, [ExcludeDate(1, 1)], 12345678);
      expect(result, isNotNull);
      expect(result.timestamp, 12345678);
      ...
      expect(
          ...
          jsonEncode({
            "id": "test_user_id",
            "description": jsonEncode(trashData),
            "globalExcludes": [
              {"month": 1, "date": 1}
            ],
            "platform": "web",
    ...
    test(
        "updateTrashDataでレスポンスのステータスコードが200でレスポンスボディにtimestampが無い場合,TrashUpdateResultのタイムスタンプが-1,UpdateResultがERRORであること",
        () async {
      when(httpClient.post(any,
              body: anyNamed("body"), headers: anyNamed("headers")))
          .thenAnswer((request) async {
  ...
  group("syncTrashData", () {
    test("syncTrashDataで正常に同期できること", () async {
      when(httpClient.get(any, headers: anyNamed("headers")))
        ...
        return Response(
            ...
            ']", "timestamp": 12345678, "platform": "ios"}',
            200,
      ...
      expect(result.globalExcludes, isEmpty);

      final captured = verify(
              httpClient.get(captureAny, headers: captureAnyNamed("headers")))
          .captured;
      expect(captured[0],
          Uri.parse("https://example.com/sync?user_id=test_user_id"));
      expect(captured[1], {
    ...
    test(
        "syncTrashDataでレスポンスのステータスコードが200以外の場合、TrashSyncResultのallTrashDataListが空、タイムスタンプが-1、SyncResultがERRORであること",
        () async {
      when(httpClient.get(any, headers: anyNamed("headers")))
          .thenAnswer((request) async {
        return Response('{"error": "error"}', 400);
    ...
    test(
        "Apiレスポンスが200でTrashApiSyncDataResponseのデコードに失敗した場合、TrashSyncResultのallTrashDataListが空、タイムスタンプが-1、SyncResultがERRORであること",
        () async {
      when(httpClient.get(any, headers: anyNamed("headers")))
          .thenAnswer((request) async {
        return Response(
  ...
  group("initialize", () {
    test("initializeが繰り返し呼ばれた場合はエラーが発生すること", () async {
      expect(() => TrashApi.initialize(appConfigProvider, httpClient),
          throwsStateError);
    });
  });
}
