...
void main() {
  ...
  group('globalExcludeDates', () {
    test('データが無い場合は空配列が返却される', () async {
      when(sharedPreferences.getStringList('GLOBAL_EXCLUDES')).thenReturn(null);
      final result = await repository.readGlobalExcludeDates();
      expect(result, isEmpty);
    });

    test('保存済みデータを正しく読み込める', () async {
      final input = [ExcludeDate(1, 1), ExcludeDate(12, 31)];
      when(
        sharedPreferences.getStringList('GLOBAL_EXCLUDES'),
      ).thenReturn(input.map((e) => jsonEncode(e.toJson())).toList());

      final result = await repository.readGlobalExcludeDates();

      expect(result.length, 2);
      expect(result[0].month, 1);
      expect(result[0].date, 1);
      expect(result[1].month, 12);
      expect(result[1].date, 31);
    });

    test('例外日配列を保存できる', () async {
      final input = [ExcludeDate(2, 29), ExcludeDate(3, 1)];
      when(
        sharedPreferences.setStringList('GLOBAL_EXCLUDES', any),
      ).thenAnswer((_) async => true);

      final result = await repository.writeGlobalExcludeDates(input);

      expect(result, isTrue);
      verify(
        sharedPreferences.setStringList(
          'GLOBAL_EXCLUDES',
          input.map((e) => jsonEncode(e.toJson())).toList(),
        ),
      ).called(1);
    });
  });
