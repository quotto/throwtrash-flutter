...
ゴミ出し予定検索 API を利用し、郵便番号または住所からゴミ出し予定を AI取り込みする機能を実装する。
...
- 取り込み結果のローカル通知は、アプリがバックグラウンドにある状態でも表示し、一覧画面表示やアプリ復帰を通知表示の条件にしない。
- 現在登録済みのゴミ出し予定は、正常レスポンス反映時に全削除して API レスポンス内容へ置き換える。
- 反映後はリモート同期待ち状態にする。
- 初回起動時の AI取り込みダイアログは、実行・キャンセルに関わらず一度閉じたら次回以降のアプリ起動では表示しない。
- ゴミ出し予定一覧画面で表示する取り込み完了・エラーメッセージは取り込み処理直後のみ表示し、次回以降は表示しない。
- 取り込み完了・エラーメッセージの背景色は、正常系・異常系でアプリ全体の共通定義を使い分ける。
...
7. 非同期実行と結果通知を実装する。
   ...
   - 処理完了時にローカル通知を表示する。
   - エラー時もローカル通知を表示する。
   - アプリがバックグラウンドに移行した後も取り込み結果通知が表示されるよう、通知初期化と通知表示の実行経路を見直す。
   - ゴミ出し予定一覧画面の一度きりメッセージはアプリ内確認用として維持し、通知表示のトリガーにはしない。
8. ゴミ出し予定一覧画面でメッセージを表示する。
   - 正常レスポンス反映後は取り込み完了メッセージを表示する。
   - エラーの場合はエラー内容を表示する。
   - メッセージは取り込み処理直後のみ表示し、表示後に消す。
   - 正常系・異常系の背景色は共通定義を利用する。
9. CI/CD を修正する。
   - AI取り込みで利用する API のエンドポイントを config で、API キーを `--dart-define` で渡せるようにする。
...
6. UI と初回表示状態を実装する。
7. 通知とゴミ出し予定一覧画面メッセージを実装する。
8. UI と通知タイトルの `自動取り込み` 表記を `AI取り込み` に更新する。
9. バックグラウンド移行後の完了・失敗通知表示を実装し、通知表示がアプリ復帰に依存しないことを検証する。
10. CI/CD を更新する。
11. `dart run build_runner build --delete-conflicting-outputs` を必要に応じて実行する。
12. `flutter test` と `flutter analyze` を実行する。
13. タスク一覧を更新し、実装内容とドキュメントの差異を確認する。

## 未確定事項

- 既存データ削除時にグローバル例外日も削除するか。
  - 初期案: API 仕様に含まれないため維持する。
- API の処理が数分かかる場合、アプリがバックグラウンドに移行した後も取り込み処理とローカル通知表示を継続できる実行方式を確認する。
  - 今回の要件はバックグラウンド状態での通知表示を対象とする。OS によるプロセス終了・サスペンド時の扱いは実装方式の検討時に明確化する。
