# AGENTS.md
- このドキュメントはプロジェクト固有の開発ガイドラインです。
- 共通のガイドライン（通常は開発端末の~/配下）がある場合はそちらを優先してください。

## プロジェクトの前提
- このプロジェクトはFlutterを用いたモバイルアプリケーション開発を前提とする。
- ただしビルド/デプロイはiOSアプリケーションを対象としており、Androidアプリケーションのビルド/デプロイは対象外とする。
...
- work/tasks/agents 内のタスクリストはエージェント間で共有可能なように、各worktrees/work/tasks内にシンボリックリンクを展開する。
...
- ソースコードの修正後、`dart analyze`を使ってコードスタイルを確認し、自動修正可能な問題は`dart fix`で自動修正を行う。自動修復されない問題は修正する。
